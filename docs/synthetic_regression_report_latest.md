...
## XFAIL — test_inn_spaced_known_gap_001

Known gap: ИНН с пробелами должен в будущем скрываться как INN, но текущий детектор может его не поддерживать.

**Known gap:** Текущий строгий INN regex, вероятно, не поддерживает разделители внутри ИНН.
...
- `expected_contains`: `ИНН: [INN_1]`; actual: ``
- `expected_not_contains`: `473 254 765 214`; actual: ``
- `expected_replacement_type`: `INN`; actual: `[]`

## XFAIL — test_inn_ocr_letter_known_gap_001

Known gap: OCR-буква З вместо цифры 3 рядом с меткой ИНН должна в будущем попадать в suspect/review.

**Known gap:** Нужен отдельный OCR-normalization/suspect слой для букв, похожих на цифры.
...
- `expected_contains`: `ИНН: [OCR_SUSPECT_INN_1]`; actual: ``
- `expected_not_contains`: `47325476521З`; actual: ``
- `expected_replacement_type`: `OCR_SUSPECT_INN`; actual: `[]`
